...
def recalculate_costs(apps, schema_editor):
    source_doc_model = apps.get_model("extractor", "SourceDocument")
    # Verified July 2025 Gemini 2.5 rates (standard baseline model): input = $0.30/1M, output = $2.50/1M
    docs_to_update = []
    for doc in source_doc_model.objects.filter(status="COMPLETED"):
    ...
    if docs_to_update:
        source_doc_model.objects.bulk_update(docs_to_update, ["cost_usd"])
