...
CI installs Python security scanners in an isolated environment if their dependency graph differs from the application runtime. Workflow-security scanners check out the repository before auditing it so an empty runner workspace cannot produce a false failure. AST-Grep is invoked through its pinned official npm CLI because the similarly named Python package does not expose a command-line executable. This preserves reproducible application tests and keeps all scanner results blocking.
