...
CI installs application checks from `requirements-dev.txt` and keeps dependency-divergent Python security scanners such as Semgrep in an isolated environment. Workflow-security scanners check out the repository before auditing it so an empty runner workspace cannot produce a false failure. Cloud Build resolves the pushed image digest once and uses that same immutable reference for both Syft SBOM generation and Artifact Registry attachment. AST-Grep is invoked through its pinned official npm CLI because the similarly named Python package does not expose a command-line executable. This preserves reproducible application tests and keeps all scanner results blocking.
