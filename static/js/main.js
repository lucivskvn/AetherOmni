...
function initializeLibraryFilter() {
    ...
    filterInput.addEventListener('input', () => {
        ...
        const rows = document.querySelectorAll('.files-panel table tbody tr');
        let visibleCount = 0;

        rows.forEach(row => {
            const cells = row.querySelectorAll('td');
            if (cells.length < 2) return; // Skip empty state or special rows
            if (row.id === 'table-filter-empty-row') return; // Skip our own empty row
        ...
        if (visibleCount === 0 && rows.length > 0 && query !== '') {
            ...
            if (emptyTextEl) {
                emptyTextEl.textContent = `No documents match your filter term "${filterInput.value}".`;
            }
