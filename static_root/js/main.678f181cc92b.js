...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 8. Non-disruptive Live Status Polling Loop
...
/**
 * Asynchronous Background Status Poller.
...
function initializeStatusPoller() {
    ...
    function updateDashboardStats(stats) {
        ...
        if (tokenContainer) {
            const tokenValue = tokenContainer.querySelector('div div');
            if (tokenValue) {
                tokenValue.textContent = formatCompact(stats.total_tokens);
            }
            const tokenSub = tokenContainer.querySelector('div div:nth-child(2)');
            if (tokenSub) {
    ...
    function updateDocumentDetailScreen(data) {
        ...
        if (detailLang && currentDoc.language !== 'Unknown') {
            detailLang.textContent = currentDoc.language;
        }

        const detailAuthor = document.getElementById('detail-author');
        if (detailAuthor && currentDoc.author !== 'Unknown') {
